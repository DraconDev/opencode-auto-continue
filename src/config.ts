...
export const DEFAULT_CONFIG: PluginConfig = {
  ...
  shortContinueMessage: "Continue. Use the TodoWrite tool to create todo items for any untracked work before starting it.{todoMdInstruction}",
  continueWithPlanMessage: "You were creating a plan. Finish the plan, **use the TodoWrite tool to create todos for each planned item**, then start executing the first one. Do not stop to ask for confirmation.{todoMdInstruction}",
  continueMessage: "Continue from where you left off. Do not ask for permission — just proceed with the next step. For each task, write a test FIRST that defines the expected outcome, then implement the code to make it pass. Run your project's test command to verify after each implementation. If you discover new work, **use the TodoWrite tool to create a todo for it before starting**.{todoMdInstruction}",
  continueWithTodosMessage: "Continue from where you left off. You have {pending} open task(s): {todoList}. Work through these tasks. **Write a test first for each task, then implement the code.** Run your project's test command to verify after each implementation. **You must use the TodoWrite tool to create todos for any new work you discover before starting it** — do not do untracked work. Do not ask for permission or confirmation.{todoMdInstruction}",
